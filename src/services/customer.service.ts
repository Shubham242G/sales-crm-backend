...
export const usecustomerApiHook = () => {
    ...
    const addCustomer = async (obj: any) => {
        ...
        return axios.post<GeneralApiResponse<ICustomer>>(`${BASE_URL}${prefix}/`, obj);
    };
    ...
    const getCustomerById = async (id: any) => {
        return axios.get<GeneralApiResponse<ICustomer>>(`${BASE_URL}${prefix}/getById/${id}`);
    };

    const getAllCustomer = async (pagination: PaginationState, searchObj: any) => {
        const query = new URLSearchParams({
            pageIndex: String(pagination.pageIndex),
            pageSize: String(pagination.pageSize),
            ...searchObj,
        }).toString();
        return axios.get<GeneralApiResponsePagination<ICustomer>>(`${BASE_URL}/${prefix}?${query}`);
    };

    const convertEnquiry = async (id: any) => {
        return axios.post<GeneralApiResponse<ICustomer>>(`${BASE_URL}${prefix}/convert/${id}`);
    }
...
export const useCustomer = (searchObj: Record<string, any> = {}, getPaginationFromParams = true) => {
    ...
    return useQuery({
        ...
        queryFn: () => api.getAllCustomer(pagination, searchObj).then((res) => res?.data),
        initialData: {
            data: [],
            total: 0,
            message: "",
