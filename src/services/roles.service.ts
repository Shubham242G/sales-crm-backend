...
export const useRolesApiHook = () => {
    ...
    const getContactById = async (id: any) => {
        return axios.get<GeneralApiResponse<IRoles>>(`${BASE_URL}${prefix}/getById/${id}`);
    };
    ...
    const getAllRoles = async (pagination: PaginationState, searchObj: any) => {
        ...
        return axios.get<GeneralApiResponsePagination<IRoles>>(`${BASE_URL}${prefix}/?${query}`);
    };
...
export const useRoles = (searchObj: Record<string, any> = {}, getPaginationFromParams = true) => {
    ...
    return useQuery({
        ...
        queryFn: () => api.getAllRoles(pagination, searchObj).then((res: any) => res?.data),
        initialData: {
            data: [],
            total: 0,
            message: "",
