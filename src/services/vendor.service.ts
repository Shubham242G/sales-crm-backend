...
export const useVendorApiHook = () => {
    ...
    return {
        addVendor, deleteVendorById, updateVendorById, getVendorById, getAllVendor
    };
};
...
export const useVendor = (searchObj: Record<string, any> = {}, getPaginationFromParams = true) => {
    ...
    return useQuery({
        ...
        queryFn: () => api.getAllVendor(pagination, searchObj).then((res) => res?.data),
        initialData: {
            data: [],
            total: 0,
            message: "",
