...
export const useleadApiHook = () => {
  ...
  const getContactById = async (id: any) => {
    return axios.get<GeneralApiResponse<ILead>>(
      `${BASE_URL}${prefix}/getById/${id}`
    );
  };

  const convertToEnquiry = async (id: any) => {
    return axios.get<GeneralApiResponse<Inew>>(
      `${BASE_URL}${prefix}/convertToEnquiry/${id}`
    );
  };
  ...
  const getAllLead = async (pagination: PaginationState, searchObj: any) => {
    ...
    return axios.get<GeneralApiResponsePagination<ILead>>(
  ...
  const convertToContact = async (id: any) => {
    return axios.get<GeneralApiResponse<Inew>>(
      `${BASE_URL}${prefix}/convertToContact/${id}`
    );
  };
  ...
  const getCompanyName = async (fullName: string) => {
    return axios.get<GeneralApiResponse<ILead>>(
      `${BASE_URL}${prefix}/getCompanyName/${fullName}`
    );
  };
...
export const useLead = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      api.getAllLead(pagination, searchObj).then((res: any) => res?.data),
    initialData: {
      data: [],
      total: 0,
      message: "",
...
export const addLeadExel = async (obj: any) => {
  return axios.post<GeneralApiResponse>(
    `${BASE_URL}${prefix}/bulkUploadLeads`,
    obj,
    { headers: { "Content-Type": "multipart/form-data" } }
  );
};

export const useLeadName = (searchObj: Record<string, any> = {}) => {
  const api = useleadApiHook();
  return useQuery({
    queryKey: ["LeadName", searchObj],
    queryFn: () => api.getAllLeadName(searchObj).then((res) => res?.data),
    initialData: {
      data: [],
      total: 0,
      message: "",
...
export const useCompanyName = (fullName: string) => {
  ...
  return useQuery({
    ...
    queryFn: () => api.getCompanyName(fullName).then((res) => res?.data),
    initialData: {
      data: [],
      message: "",
